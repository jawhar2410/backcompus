...
exports.createEmploi = async (req, res) => {
    try {
      ...
      if (!professeurExists.matiere) {return res.status(400).json("Le professeur n'a pas de matière assignée.")}
...
exports.deleteEmploi = async (req, res) => {
    try {
        ...
        const emploi = await Emploi.findByIdAndDelete(emploiId);
...
exports.getAllEmplois = async (req, res) => {
    try {
        const emplois = await Emploi.find().populate("classe");
...
exports.getEmploiByclasse = async (req, res) => {
  try {
      ...
      if ( !classe ) return res.status(404).send({message : "classe non valide"})
       const emplois = await Emploi.find({classe :classeId }).populate('matiere' , 'id nom ').populate('classe' , 'id nom ').populate('professeur' , 'id nom prenom');
...
//// get emploi by id prof
      ...
      const emplois = await Emploi.find({professeur :profId})
...
if (!mongoose.Types.ObjectId.isValid(classeId)) {
  ...
  const emplois = await Emploi.find({ classe: classeId })
...
exports.getEmploiById = async (req, res) => {
    try {
        const { emploiId } = req.params;
        const emploi = await Emploi.findById(emploiId).populate("classe");
        if (!emploi) return res.status(404).send({ message: "Emploi not found!" });
